...
namespace SimpleFabric.Collections
{
    ...
    public class TimeoutHandler<TKey, TValue> where TValue : IExpiring<TKey>
    {
            ...
            while (true)
            {
                ...
                lock (sortedItems)
                {
                    ...
                    foreach (var entry in sortedItems)
                    {
                        ...
                        entry.Value.TimeoutHandler?.Invoke();
        ...
        /// <summary>
        /// This will update the timeout for an entry by
        /// first removing any existing timeout handlers,
        /// and then adding a new
        /// </summary>
        /// <param name="key">The key</param>
        /// <param name="value">The value</param>
        /// <param name="milliseconds">Time timeout</param>
        /// <returns>number of ticks until timeout</returns>
        public Int64 UpdateTimeout(TKey key, TValue value, int milliseconds)
        {
            RemoveTimeout(key);
